...
const acceptDepositOrder = async function (req, res, next) {
    ...
    try {
        ...
        try {
            let notification = await notificationService.user_acceptDepositOrder(user._id, depositOrder);
            engine.publicIo.to(user._id).emit(SocketEvent.NOTIFICATION, notification);
        } catch (e) {
            logger.error('admin_depositOrderService_acceptDepositOrder', `could not create notification to user`);
        }
        ...
        await session.abortTransaction();
        await session.endSession();
        logger.error('admin_depositOrderService_acceptDepositOrder', `could not accept depositOrderId=${depositOrderId} e=${e}`);
...
const denyDepositOrder = async function (req, res, next) {
    ...
    try {
        ...
        await session.abortTransaction();
        await session.endSession();
        logger.error('admin_depositOrderService_denyDepositOrder', `could not accept depositOrderId=${depositOrderId} e=${e}`);
        next(badRequestError.make(ErrorCode.ADMIN_DEPOSIT_ORDER_COULD_NOT_ACCEPT_DEPOSIT_ORDER));
    }
}
